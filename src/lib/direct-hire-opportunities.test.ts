...
test("fills only the first sparse real-result page exactly to twelve", () => {
  for (const [realRows, expectedOpportunities] of [
    [0, 12],
    [1, 11],
    [11, 1],
    [12, 0],
    ...
    assert.equal(opportunities.length, expectedOpportunities, `${realRows} real rows`);
  ...
  assert.deepEqual(
    buildDirectHireOpportunities({
      visibleRealRows: 1,
      offset: 12,
      preferences: { q: "Schreiner", loc: "Bern" },
    }),
    [],
  );
});
...
test("uses deterministic controlled roles and validated preferences", () => {
  ...
  const second = buildForRows(0);
  assert.deepEqual(first, second);
  assert.equal(new Set(first.map((entry) => entry.id)).size, 12);
  assert.equal(first[0]?.role, "CNC-Maschinist/in Holz");
...
test("never echoes strange query, location, or filter input", () => {
  ...
  assert.doesNotMatch(
    serialized,
    /JobPosting|datePosted|salary|sourceUrl|jobUrl|application|\/jobs\//i,
  );
...
test("cannot cross the public JobListing DTO boundary", () => {
  ...
  assert.throws(
    () => assertPublicJobListing(opportunity as unknown as JobListing),
    /unreviewed field/,
  );
});
...
test("SSR, API, client, schema, and sitemap keep opportunities separate", () => {
  ...
  const client = readFileSync(
    new URL("../app/_components/homepage-search.tsx", import.meta.url),
    "utf8",
  );

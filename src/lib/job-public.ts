...
import { classifySchreinerTitle, getSchreinerProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifySchreinerTitle(input.title);
  if (
  ...
  const profile = getSchreinerProfile(classification.profile);

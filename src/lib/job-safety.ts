...
export function classifySchreinerTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-schreiner-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-schreiner-title" : "broad-role-with-schreiner-context",
...
export function isValidSchreinerJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getSchreinerProfile(profile: JobSafetyProfile) {

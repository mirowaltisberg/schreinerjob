...
import { classifySchreinerTitle, isValidSchreinerJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifySchreinerTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
function matchesQuery(job: JobListing, query: string): boolean {
  ...
  const hasPublicMatch = (queryToken: string) => {
    if (queryToken === "schreiner" || queryToken === "schreinerin") {
      return true;
    }
...
export async function getJobListingById(input: {
  ...
  if (!isValidSchreinerJobId(input.id)) {
    return null;
  }

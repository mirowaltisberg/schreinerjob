...
function getDisplayJobId(job: JobListing): string {
  ...
  if (job.source === "scraped") {
    ...
    return `SCH-${hash.slice(0, 8).toUpperCase()}`;
  }
  return `SCH-${job.id.padStart(4, "0")}`;
}
...
function buildJobPostingSchema(job: JobListing): Record<string, any> {
  ...
  const schema: Record<string, any> = {
    ...
    hiringOrganization: {
      "@type": "Organization",
      name: "Arbeitgeber via schreinerjob.ch",
...
export async function generateMetadata(props: JobDetailsPageProps): Promise<Metadata> {
  ...
  if (!job) {
    return {
      title: "Stelle nicht gefunden | schreinerjob.ch",
  ...
  return {
    title: `${job.title} | schreinerjob.ch`,
    description,
    alternates: {

"""Scrape a fresh chunk of genuine Swiss joinery jobs into JSON.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_schreiner_title
...
def is_relevant_job(job: dict) -> bool:
    """Return True only for clearly joinery-related Swiss roles."""
    ...
    return classify_schreiner_title(title)["disposition"] == "ACCEPT"
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Swiss joinery jobs")

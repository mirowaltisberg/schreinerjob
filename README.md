...
Independent Swiss job discovery site for vacancies with a clear connection to the joinery trade. The production site is [schreinerjob.ch](https://schreinerjob.ch).
...
The scheduled workflow runs five isolated scrape workers with at most two workers in parallel. Workers receive no Supabase secret and upload short-lived JSON artifacts. A single publisher then validates the complete snapshot, deduplicates it, enforces absolute and retention thresholds, writes only `trade = schreiner`, prunes only that trade and verifies the stored count and trade-scoped metadata.

Publishing is disabled by default. Scheduled runs can never enter the `publish` job. A manual workflow run must supply the exact per-run input `confirm_publish = PUBLISH` **and** a repository administrator must have set the Actions variable `SCHREINER_PUBLISHING_APPROVED` to the exact value `true`. A push or scheduled scrape therefore cannot publish or prune, even when the variable and database secrets already exist.

The publisher reads the entire Schreiner inventory, including legacy IDs. A stale row can be deleted only through a batch constrained by both `trade = schreiner` and the exact validated stale-ID list; rows belonging to another trade are never in scope.

The publisher is intentionally fail-closed. Use `--dry-run` for an offline artifact check. Use `--plan` for aggregate, read-only comparison with the current Schreiner inventory. Publishing or pruning production rows requires an intentional authorized run.
